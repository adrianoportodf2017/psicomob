...
class Payment_model extends CI_Model {
    ...
    public function pagarme_payment_charge($post = "", $public_key = "", $payment_method = "credit_card") {
        require_once(APPPATH . '../vendor/autoload.php');
        $pagarme = new PagarMe\Client($post['public_key']);
        $user_details = $this->user_model->get_all_user($post['user_id'])->row_array();
        $user_address = $this->user_model->has_address($post)->row_array();
        ...
        $itens = [];
        $counter = 0;
        $link_details = $this->crud_model->get_charge_by_id($post['charge_id'])->row_array();
        $split_rules = $this->crud_model->list_profile_user_payment($post['profile_id']);
        $i = 0;
        foreach ($split_rules as $user) {
            $split[$i] = [
        ...
        try {
            ...
            if ($transaction->status == 'paid') {
                $trancasao_id = $transaction->id;
                $this->db->set('transacao_id', $trancasao_id);
                $this->db->set('payd', '1');
                $this->db->set('status', 'paid');
                $this->db->set('method_payment', $payment_method);
                $this->db->set('date_payment', strtotime(date('D, d-M-Y')));
                $this->db->where('id', $post['id']);
                $this->db->update('payment_charge_installments');
                return ['status' => 'paid'];
            } elseif ($transaction->status == 'waiting_payment') {
                $trancasao_id = $transaction->id;
                // echo '<pre>';
                ...
                $this->db->set('method_payment', $payment_method);
                $this->db->set('issuance_date', strtotime(date('D, d-M-Y')));
                $this->db->set('boleto_url', $transaction->boleto_url);
                $this->db->set('boleto_expiration_date', $transaction->boleto_expiration_date);
                $this->db->where('id', $post['id']);
                $this->db->update('payment_charge_installments');
                $this->session->set_userdata('transaction', $transaction);
                return ['status' => 'waiting_payment',
    ...
    // VALIDATE PAGAR.ME link PAYMENT
    public function pagarme_payment_link($post = "", $public_key = "", $payment_method = "credit_card") {
        require_once(APPPATH . '../vendor/autoload.php');
        $pagarme = new PagarMe\Client($post['public_key']);
        $user_details = $this->user_model->get_all_user($post['user_id'])->row_array();
        $user_address = $this->user_model->has_address($post)->row_array();
        ...
        $itens = [];
        $counter = 0;
        $link_details = $this->crud_model->get_link_by_id($post['link_id'])->row_array();
        $split_rules = $this->crud_model->list_profile_user_payment($post['profile_id']);
        $parcelas = $this->payment_model->checkar_taxa_juros_link($post['public_key'], $post['amount'], $post['link_id']);
        if ($link_details['discounted_price'] > '0' && $link_details['discounted_price'] < $link_details['price'] || $link_details['discounted_price'] > 0 && $link_details['discounted_price'] < $course_details['price']) {
            $page_data['amount_to_pay'] = $link_details['discounted_price'];
            $desconto = $link_details['price'] - $link_details['discounted_price'];
            $page_data['total'] = $link_details['price'] - $desconto;
            $page_data['amount_to_pay'] = $page_data['total'];
        } else {
            $page_data['amount_to_pay'] = $link_details['price'];
        }
        $i = 0;
        foreach ($split_rules as $user) {
            $split[$i] = [
